...
\begin{proof}
  ...
  Противоречие.
\end{proof}
\subsection{Теорема о приближении интеграла вдоль кусочно гладкой кривой интегралом вдоль ломаной}
\begin{theorem}
  Пусть $\gamma$~--- кусочно гладкая Жорданова кривая, $f \in \C(D)$, $\gamma \subset D$. Тогда $\forall \varepsilon > 0$ $\exists L$~--- ломаная такая, что 
  \[ \left| \int\limits_{\gamma} f(z) \d z - \int\limits_{L} f(z) \d z \right| < \varepsilon \]
\end{theorem}
\begin{proof}
  \textit{(тут возможны баги).}

  Так как $\gamma$~--- компакт, то из покрытия $\{ U_{\varepsilon(z)}(z) \subset D : \overline{U_{\varepsilon(z)}}(z) \subset D \}_{z \in \gamma}$ можно выбрать конечное подпокрытие $\displaystyle \gamma \subset \overline{\bigcup_{j = 1}^{n} U_{\varepsilon(z_{j})}(z_{j})} \subset D$.
  Пусть $D_{1} = \bigcup_{j = 1}^{n} U_{\varepsilon(z_{j})}(z_{j})$~--- ограниченная область, $\overline{D_{1}} \subset D$. $\overline{D_{1}}$~--- компакт, $f \in C(D) \implies$ $f$ равномерно непрерывна на $\overline{D_{1}}$. Значит, $\forall \varepsilon > 0$ $\exists \delta$ $\forall z_{1}, z_{2} \in \overline{D_{1}}$ $|z_{1} - z_{2}| < \delta \implies \displaystyle |f(z_{1}) - f(z_{2})| < \frac{\varepsilon}{3 |\gamma|}$.
  
  $\partial D_{1}$~--- компакт, $\gamma$~--- компакт. Пусть $f(z_{1}, z_{2}) = |z_{1} - z_{2}|$. Тогда по теореме Вейерштрасса $\inf\limits_{z_{1} \in \gamma, z_{2} \in \partial D_{1}} |z_{1} - z_{2}| = d > 0$ и $d$ достигается.
  Тогда $\forall \delta < d$ $U_{\delta}(z) \subset D$ $\forall z \in \gamma$.

  Выберем $\delta_{1} = \min(\delta, d)$. $\gamma$ задается путем $\Gamma : [a, b] \to \gamma$. Выберем разбиение $T$ отрезка $[a, b]$ $a = t_{0} < t_{1} < \ldots < t_{n} = b$ такое, что $\gamma_{k} = \Gamma([t_{k - 1}, t_{k}])$ и $|\gamma_{k}| < \delta_{1}$, пусть $z_{k} = \Gamma(t_{k})$.
  Рассмотрим $L = z_{0}z_{1}\ldots z_{n}$, $l_{k} = [z_{k - 1}, z_{k}]$, $|l_{k}| < |\gamma_{k}| < \delta_{1}$. $\gamma_{k}, l_{k} \subset U_{\delta_{1}}(z_{k - 1})$.

  \begin{multline*}
    \left| \int_{\Gamma} f(z) \d z - \int_{L} f(z) \d z \right| = 
    \left| \sum_{k = 1}^{n} \int_{\gamma_{k}} f(z) \d z - \int_{\gamma_{k}} f(z_{k}) \d z + \int_{\gamma_{k}} f(z_{k}) \d z - \int_{l_{k}} f(z) \d z \right| \leq \\ \leq
    \left| \text{$\int_{\gamma_{k}} f(z_{k}) \d z$ интеграл от константы, $\int_{\gamma_{k}} \to \int_{l_{k}}$} \right| \leq \\ \leq
    \sum_{k = 1}^{n} \left| \int_{\gamma_{k}} f(z) - f(z_{k}) \d z \right| +  \left| \int_{l_{k}} f(z_{k}) - f(z) \d z \right| \leq
    \sum_{k = 1}^{n} \frac{\varepsilon}{3 |\gamma|} \cdot |\gamma_{k}| + \frac{\varepsilon}{3 |\gamma|} \cdot |l_{k}| =\\=
    \frac{\varepsilon}{3} + \frac{\varepsilon |L|}{3 |\gamma|} \leq 
...
\subsection{Теорема о существовании первообразной в области}
\begin{theorem}
  Если $f \in C(D)$ и $\oint\limits_{\g}f(z)\d z = 0$ $\forall\g$ - замкнутого пути, то $\exists F$ - первообразная $f$ в $D$.
\end{theorem}
\begin{proof}
  Зафиксируем $a \in D$. Пусть $\displaystyle F(z) = \int_{\gamma_{a\zeta}} f(\zeta) \d \zeta$, где $\gamma_{a\zeta}$~--- произвольный кусочно гладкий простой путь от $a$ к $z$.

  Покажем, что это первообразная. Пусть $z \in D$. Тогда $\exists \delta : U_{\delta}(z) \subset D$, тогда $\forall h : |h| < \delta$ $z + h \in U_{\delta}(z)$. Тогда $\displaystyle F(z + h) = \left( \int_{\gamma_{az}} + \int_{[z, z + h]} \right)f(\zeta) \d \zeta$.
  \[ 
    \left| \frac{F(z + h) - F(z)}{h} - f(z) \right| = 
    \left| \frac{1}{h} \int_{[z, z + h]} f(\zeta) \d \zeta - \frac{1}{h} \int_{[z, z + h]} f(z) \d \zeta \right| \overset{h \to 0}{\longrightarrow} 0 
  \]
\end{proof}
...
\end{proof}
\subsection{Теорема о связи между голоморфными и гармоническими функциями}
\begin{theorem}
  Пусть $f \in \O(D)$. Тогда $u(x, y) = \Rea f(z)$ гармонична в $D$. 
  
  Обратно, если $D$ односвязная и $u : D \to \R$ гармонична, то существует единственная с точностью до мнимой константы функция $f \in \O(D)$ такая, что $\Rea f = u(x, y)$ для всех $z \in D$.
\end{theorem}
\begin{proof}
  Так как $f \in \O(D)$, то $f \in C^{\infty}(D)$, тогда $u, v \in C^{\infty}(D)$ и удовлетворяют условию Коши-Римана. $u_{x} = v_{y}$, $u_{y} = -v_{x}$. Тогда $u_{xx} + u_{yy} = (v_{y})_{x} + (-v_{x})_{y} = 0$. Тогда $u$ гармонична в $D$ (и $v$ тоже).

  Обратно, рассмотрим $g = u_{x} - iu_{y}$. $g$ $\R$-дифференцируема в $D$, так как $u \in C^{2}(D)$, и удовлетворяет условию Коши-Римана. Действительно, $(u_{x})_{x} = (-u_{y})_{y} = -u_{yy}$ ($u$ гармонична), $(u_{x})_{y} = -(-u_{y})_{x} = u_{yx}$ ($u \in C^{2}(D)$). Тогда $g \in \O(D)$, но $D$~--- односвязная область, поэтому у $g$ есть первообразная в $D$. $G = U + iV$, $G' = g = U_{x} + iV_{x} = U_{x} - iU_{y}$ (Коши-Риман). Тогда $U_{x} - u_{x} = (U - u)_{x} = 0 = (U - u)_{y} = U_{y} - u_{y}$. То есть $U - u = C$, $C = const$, скажем $C = 0$, получим $\Rea f = u$.  
\end{proof}
...
\begin{proof}
  \textit{(тут возможны баги).}
  \begin{enumerate}
    \item $\forall a\in D$, так как $\overline{U_{R}(a)}$ - компакт, то $f_{n} \overset{\overline{U_{R}(a)}}{\rightrightarrows} f \implies f \in C(\overline{U_{r}(a)}) \implies f$ непрерывна в точке $a$. Значит $f \in C(D)$.

    Пусть $\Delta$~--- треугольник $\colon \overline{\Delta} \subset D$. Тогда 
    \[\oint_{\partial \Delta} f(z) \d z = \oint_{\partial \Delta} \Lim{n}{\infty} f_n(z) \d z = |\text{равн. сход.}| = \Lim{n}{\infty} \underbrace{\oint_{\partial \Delta} f_n(z) \d z}_{= 0 \text{ по лемме Гурса}} = 0\]
    Значит по теореме Мореры $f \in \O(D)$.
    \item Пусть $K\subset D$ - компакт. Докажем, что $\forall a \in K \q \exists \overline{U_{r}(a)} \colon f_{n}^{(N)} \overset{\overline{U_{r}(a)}}{\rightrightarrows} f^{(N)}$.

      Рассмотрим $R \colon \overline{U_{R}(a)} \subset D$, возьмем $0 < r < R$.
    Покажем, что есть равномерная сходимость:
    \begin{multline*}
      \sup_{z \in \overline{U_{r}(a)}} |f_{n}^{(N)}(z) - f^{(N)}(z)| = 
      |\text{ИФК}| = 
      \sup_{z \in \overline{U_{r}(a)}}\left|\frac{N!}{2 \pi i} \oint_{|\zeta - a| = R} \frac{f_{n}(\zeta) - f(\zeta)}{(\zeta - z)^{N + 1}} \d \zeta\right|\leq \\\leq\frac{N!}{2 \pi} \sup_{z \in \overline{U_{r}(a)}} \left|\oint_{|\zeta - a| = R} \frac{f_{n}(\zeta) - f(\zeta)}{(\zeta - z)^{N + 1}} \d \zeta\right| \leq
      \frac{N!}{2 \pi} \sup_{z \in \overline{U_{r}(a)}} 2\pi R \cdot \sup_{|\zeta - a| = R} \frac{|f_{n}(\zeta) - f(\zeta)|}{|(\zeta - z)^{N + 1}|} \leq\\\leq 
      \frac{N! R}{(R-r)^{N+1}} \underbrace{\sup_{|\zeta - a| = R} |f_{n}(\zeta) - f(\zeta)|}_{\text{есть $\rightrightarrows$ на компакте}}
    \end{multline*}
    Теперь выберем конечное подпокрытие $K$ c $\bigcup_{j = 1}^{M} \overline{U_{r_j}(a_{j})} \implies f_n^{(N)}\overset{K}{\rightrightarrows}f^{(N)}$ 
  \end{enumerate}
\end{proof}
